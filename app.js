...
var gitter = {
 ...
 fetchMessages: function(rooms, token, cb) {
    this.fetch('/api/v1/rooms/' + room.id + '/chatMessages', token, function(err, messages) {
	cb(err, messages);
    });
  }
};
...
//////// I want to modify this part of the code to fetch room messages//////////

app.get('/home', function(req, res) {
  if (!req.user) return res.redirect('/');

  gitter.fetchMessages(req.rooms, req.session.token, function(err, messages) {
    if (err) return res.send(500);
